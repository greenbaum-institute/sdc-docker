...
function pushImageLayers(job, cb) {
    ...
    job.log.info('job.params.req_id: ', job.params.req_id);
    ...
    imgapi.adminPushDockerImage(job.params, function (err, res) {
        job.log.info('adminPushDockerImage callback fired');
        if (err) {
        ...
        res.on('end', function onEnd() {
            job.log.info('pushImageLayers res.end received');
            queue.close();

            // Wait for queue to finish before ending the task
            queue.on('end', function () {
                job.log.info('pushImageLayers queue.end received');
                if (queueError) {
                    cb(queueError);
                } else {
