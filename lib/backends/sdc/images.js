...
function pushImage(opts, callback) {
    ...
    function errorAndEnd(err, job) {
        ...
        if (err.code === 'ENOTFOUND') {
             ...
             *  Using default tag: latest
             *  Pushing repository nope.example.com/nope
             *  Error pushing image: (ENOTFOUND) nope.example.com host not found
             *
             * err={ [Error: getaddrinfo ENOTFOUND] code: 'ENOTFOUND' }
             */
            /* END JSSTYLED */
    ...
    function _waitForPushJob(_, next) {
        common.waitForJob(req.wfapi, jobUuid, function (err, job) {
            log.info('push job finished, err: %s', err);
            if (err) {
                errorAndEnd(err, job);
            }
 ...
 */
function imgTagsForRepoName(opts, callback) {
    assert.object(opts, 'opts');
...
module.exports = {
    ...
    pullImage: pullImage,
    pushImage: pushImage,
    imgTagsForRepoName: imgTagsForRepoName,
    tagImage: tagImage
};
