...
function pushImage(opts, callback) {
    ...
    function errorAndEnd(err, job) {
        ...
        log.info({err: err, repoAndTag: repoAndTag,
            jobUuid: job && job.uuid}, 'imagePush job error');

        var payload;
        if (err.code === 'ENOTFOUND') {
            /*
             * `docker push nope.example.com/nope` yields:
             *      { [Error: getaddrinfo ENOTFOUND] code: 'ENOTFOUND' }
             */
            payload = {
                status: format('Error pushing image: (%s) %s host not found',
                    err.code, rat.index.name)
            };
        } else if (err.code === 'UnauthorizedError') {
            payload = {
                status: format('Unauthorized error from registry %s trying to '
                    + 'push %s (%s)', rat.index.name, rat.remoteName,
                    req_id)
            };
        } else if (err.code === 'NotImplemented') {
            // E.g. OAuth auth to a Docker Registry before DOCKER-771.
            payload = {
                status: format('Could not push to registry %s: %s',
                    rat.index.name, err.message)
            };
        } else {
            var regErr;
            try {
            ...
            if (!regErr || !regErr.errors) {
                payload = {
                    status: format('Error pushing image: Internal error (%s)',
                        req_id)
                };
            } else if (regErr.errors.length === 1) {
                var code = regErr.errors[0].code;
                if (code === 'MANIFEST_UNKNOWN') {
                    ...
                    payload = {
                        status: format('Error pushing image: (%s) %s (%s)',
                            code, regErr.errors[0].message, req_id)
                    };
                }
            } else {
                var errmsgs = regErr.errors.map(function (e) {
                    ...
                    .join(', ');
                payload = {
                    status: format('Error pushing image: %s (%s)',
                        errmsgs, req_id)
                };
            }
        }
    ...
    // 1. Load image/layer history.
    function _loadImageHistory(ctx, next) {
        getImageHistory({
            account: req.account,
            app: req.app,
            img: opts.image,
    ...
    function _createPushJob(ctx, next) {
        // Munge history to be in the wanted docker format.
        ctx.history = ctx.history.map(function _updateHistory(img) {
            var h = img.toJSON();
            // Convert created number (epoch) to ISO timestamp.
            h.created = new Date(h.created).toISOString();
            return h;
        });
        log.debug('ctx.history: %j', ctx.history);

        var jobOpts = {
            account_uuid: req.account.uuid,
            configs: JSON.stringify(ctx.history),
            headers: { 'x-request-id': req_id },
            image_uuid: opts.image.image_uuid,
